...
#
# Version 3.05, last edited 2019-08-29 at 04:04
...
echo by default, /usr/local/langtran
...

{
    ...
    ListMyFiles
} | sed 's/\r*$/\r/' > FileLists/LTF_$FirstTime.txt
...
} | sed 's/\r*$/\r/' > Diffs/diff_$TimeNow.txt
# Check for pruning internal system files from diff file.
#
if [ "$PruneDiffs" != "" ]
then
    sed -e "/diff_[_0-9]*\.txt/d" -e "/LTF_[_0-9]*\.txt/d" -e "/FoldersUpdated.txt/d" -e 's/\r*$/\r/' $PrevList > Progs/Prev.txt
    PrevList=Progs/Prev.txt
    sed -e "/diff_[0-9_]*\.txt/d" -e "/LTF_[_0-9]*\.txt/d" -e "/FoldersUpdated.txt/d" -e 's/\r*$/\r/' $NewList > Progs/NewList.txt
    NewList=Progs/NewList.txt
fi

diff -U $DiffContext -I ' 0 .*/' -I "Kill_LTupdate.sh" $PrevList $NewList | 
    sed 's/\r*$/\r/' >> Diffs/diff_$TimeNow.txt
